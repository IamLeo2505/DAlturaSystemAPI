if DB_ID('SystemAlturaCoffee') is not null
begin 
   use master
   drop database SystemAlturaCoffee
end
...
--========procedimiento buscar usuario por nombre  30//////////////////////
...
---procedimiento de listar ventas
 ...
 where idventa=@idventa
 go
 --actualizar eztock al guardar una comprea
 create proc [dbo].[spactualizar_stock]
@idproducto int,
@cantidad int
as
...
where idproducto =@idproducto
go
-----procedimiento Disminuir stock cuando realizamos una venta
...
---restablecer stock devolver el stock cuado se eliminar una venta 
...
---restablecer stock restar el stock cuado se eliminar una compra
...
BEGIN 
   ...
   IF UPDATE(estado)
   BEGIN 
     UPDATE p SET p.stock=p.stock-dc.cantidad FROM producto p
	 INNER JOIN detallecompra AS dc
	 ON P.idproducto=dc.idproducto
	 INNER JOIN inserted AS i
	 ON i.idcompra=dc.idcompra
	 WHERE i.estado='ANULADO';
...
create proc spconsultar_fechavencimiento
as
select producto.idproducto,producto.codigo,producto.nombre,producto.descripcion,
producto.f_ingreso,producto.f_vencimiento,producto.precio_compra,producto.precio_venta,
producto.stock,producto.estado,producto.idcategoria,categoria.descripcion as categoria
from categoria INNER JOIN producto on categoria.idcategoria=producto.idcategoria
where f_vencimiento <=DATEADD(DAY , 30 ,GETDATE())
OR f_vencimiento< GETDATE();
go
----reportefactura
create proc spreporte_factura
@idventa int
as
select venta.idventa,empleado.nombre+' '+empleado.apellidos as empleado,
cliente.nombre+' '+cliente.apellidos as cliente,cliente.telefono,cliente.dni,
venta.fecha,venta.tipo_documento,venta.serie,venta.num_documento,
venta.subtotal,venta.igv,venta.total,producto.nombre as nomproducto,
detalleventa.cantidad,detalleventa.precio,detalleventa.total as totalparcial
from detalleventa INNER JOIN venta
on detalleventa.idventa = venta.idventa INNER JOIN producto
on producto.idproducto=detalleventa.idproducto INNER JOIN cliente
on venta.idcliente=cliente.idcliente INNER JOIN usuario
on venta.idusuario=usuario.idusuario INNER JOIN empleado
on usuario.idempleado=empleado.idempleado
...
insert into empleado values('edgar','flores lopez','32344556','954565456','av.vernales234','ACTIVO')
insert into usuario values('admin','admin','Administrador','ACTIVO',1)
